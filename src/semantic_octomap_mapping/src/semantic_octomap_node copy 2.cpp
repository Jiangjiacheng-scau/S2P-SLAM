...
class SemanticLineFactor : public gtsam::NoiseModelFactor1<gtsam::Pose3> {
    private:
        ...
        double measured_dist_;  
    
    public:
        SemanticLineFactor(gtsam::Key key, double measured_angle, double measured_dist, gtsam::SharedNoiseModel model)
            : gtsam::NoiseModelFactor1<gtsam::Pose3>(model, key), 
              measured_angle_(measured_angle), measured_dist_(measured_dist) {}
    
        gtsam::Vector evaluateError(const gtsam::Pose3& pose, boost::optional<gtsam::Matrix&> H = boost::none) const override {
            double global_yaw = pose.rotation().yaw();
            double line_angle_world = global_yaw + measured_angle_;
            double error = std::sin(line_angle_world); 
    
            if (H) {
                gtsam::Matrix H_pose = gtsam::Matrix::Zero(1, 6);
                double deriv = std::cos(line_angle_world);
                H_pose(0, 2) = deriv; 
                *H = H_pose;
            }
            return gtsam::Vector1(error);
    ...
    void predictAndUpdate(ros::Time now, bool leg_trigger_active, 
        ...
        if (!is_ferrying) {
            ...
            if (has_line) {
                ...
                double dynamic_sigma = std::max(0.01, std::min(0.5, 0.1 * (30.0 / (line_res.inliers + 1.0)))); 
                auto gaussian_model = noiseModel::Diagonal::Sigmas(Vector1(dynamic_sigma));
                SemanticLineFactor tmp_factor(X(key_index_), final_angle, line_res.distance, gaussian_model);
                if (std::abs(tmp_factor.evaluateError(pred_pose)(0))/dynamic_sigma < 3.0) {
                    new_factors.add(boost::make_shared<SemanticLineFactor>(X(key_index_), final_angle, line_res.distance, 
                        noiseModel::Robust::Create(noiseModel::mEstimator::Huber::Create(1.345), gaussian_model)));
                }
            }
    ...
    class SemanticOctomapServer {
        public:
            ...
            void syncCallback(const sensor_msgs::ImageConstPtr& depth_msg, const sensor_msgs::ImageConstPtr& rgb_msg) {
                ...
                double dt_vis = 0.0;
                if (last_vis_time_.toSec() > 0) {
                    dt_vis = (depth_msg->header.stamp - last_vis_time_).toSec();
                }
                last_vis_time_ = depth_msg->header.stamp;
            
                cv::Mat gray_img;
                cv::cvtColor(cv_rgb_ptr->image, gray_img, cv::COLOR_BGR2GRAY);
                
                // [新增] 预处理结束时间 (t1)
                auto t1 = std::chrono::high_resolution_clock::now();
            
                bool has_vel = false;
                gtsam::Vector3 v_flow(0,0,0);
                double flow_conf = 0.0; 
                double flow_depth = 0.0; 
            
                if (dt_vis > 0 && dt_vis < 0.2) { 
                        auto flow_res = flow_estimator_.estimate(gray_img, cv_depth_ptr->image, dt_vis);
                        if (std::get<1>(flow_res) > 0.001) { 
                            has_vel = true;
                            v_flow = std::get<0>(flow_res); 
                            flow_conf = std::get<1>(flow_res); 
                            flow_depth = std::get<2>(flow_res); 
                        }
                }
            
                tf::StampedTransform transform;
                try {
                    if (!tf_listener_.canTransform(config.global_frame_id, depth_msg->header.frame_id, depth_msg->header.stamp)) {
                        return; 
                    }
                    tf_listener_.lookupTransform(config.global_frame_id, depth_msg->header.frame_id, depth_msg->header.stamp, transform);
                } catch (...) { return; }
                
                static gtsam::Rot3 R_fix = gtsam::Rot3::Rx(M_PI / 2.0); 
                gtsam::Rot3 R_calib = semantic_odom_.calibrator_.getCorrectionRotation();
                gtsam::Rot3 R_total = R_calib.compose(R_fix);
            
                gtsam::Pose3 T_w_c_raw(
                ...
                gtsam::Pose3 world_to_cam_raw = T_w_c_raw.inverse();
            
                cv::Mat depth_img = cv_depth_ptr->image;
                cv::Mat rgb_img_raw = cv_rgb_ptr->image; 
                cv::Mat rgb_img = applyLowLightEnhancement(rgb_img_raw);
                
                cv::Mat rgb_clean_for_viz = rgb_img.clone(); 
                
                cv::Mat hsv_img; 
                cv::cvtColor(rgb_clean_for_viz, hsv_img, cv::COLOR_BGR2HSV); 
            
                // ================= [PAPER VISUALIZATION PIPELINE START] =================
                static int viz_counter = 0;
                if (viz_counter++ % 2 == 0) { 
                    float fx = config.cam_fx, fy = config.cam_fy; 
                    float cx = config.cam_cx, cy = config.cam_cy;
            
                    // --- [Step 1] Geometric Filtering ---
                    cv::Mat viz_height = cv::Mat::zeros(rgb_clean_for_viz.size(), CV_8UC3);
                    cv::Mat mask_geo_cage = cv::Mat::zeros(rgb_clean_for_viz.size(), CV_8UC1);
                    
                    for (int v = 0; v < depth_img.rows; v+=2) {
                        for (int u = 0; u < depth_img.cols; u+=2) {
                            unsigned short d = depth_img.at<unsigned short>(v, u);
                            if (d < 300 || d > 8000) continue;
                            
                            float z_cam = d * 0.001f;
                            float x_cam = (u - cx) * z_cam / fx;
                            float y_cam = (v - cy) * z_cam / fy;
                            
                            gtsam::Point3 p_c(x_cam, y_cam, z_cam);
                            gtsam::Point3 p_w = T_w_c_raw * p_c; 
                            
                            float v_norm = (float)v / depth_img.rows;
                            float ground_offset = config.ground_offset_k * pow(v_norm, 2);
                            float h_w = p_w.z() + (z_cam * 0.18) + ground_offset;
            
                            if (h_w < 0.4) { 
                                cv::circle(viz_height, cv::Point(u,v), 2, cv::Scalar(0, 255, 0), -1); 
                            } 
                            else if (h_w < config.z_obstacle_thresh) {
                                cv::circle(viz_height, cv::Point(u,v), 2, cv::Scalar(0, 0, 255), -1); 
                                mask_geo_cage.at<uchar>(v,u) = 255;
                            }
                        }
                    }
                    pub_viz_step1_height_.publish(cv_bridge::CvImage(depth_msg->header, "bgr8", viz_height).toImageMsg());
            
                    // --- [Step 2] Color Rejection ---
                    cv::Mat mask_red1, mask_red2, mask_hsv;
                    cv::inRange(hsv_img, cv::Scalar(0, 30, 20), cv::Scalar(20, 255, 255), mask_red1);
                    cv::inRange(hsv_img, cv::Scalar(150, 30, 20), cv::Scalar(180, 255, 255), mask_red2);
                    cv::bitwise_or(mask_red1, mask_red2, mask_hsv);
            
                    cv::Mat viz_color = cv::Mat::zeros(rgb_clean_for_viz.size(), CV_8UC3);
                    rgb_clean_for_viz.copyTo(viz_color, mask_hsv); 
                    pub_viz_step2_hsv_.publish(cv_bridge::CvImage(depth_msg->header, "bgr8", viz_color).toImageMsg());
            
                    // --- [Step 3] Morphological Filtering ---
                    cv::Mat mask_combined = mask_geo_cage.clone(); 
                    int split = mask_combined.rows / 2;
                    cv::Mat roi_top = mask_combined(cv::Rect(0, 0, mask_combined.cols, split));
                    cv::Mat roi_bot = mask_combined(cv::Rect(0, split, mask_combined.cols, mask_combined.rows - split));
                    
                    cv::Mat k_smooth = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(5, 5));
                    cv::Mat k_connect = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 9)); 
            
                    cv::morphologyEx(roi_top, roi_top, cv::MORPH_DILATE, k_connect);
                    cv::morphologyEx(roi_bot, roi_bot, cv::MORPH_DILATE, k_connect);
                    cv::morphologyEx(roi_top, roi_top, cv::MORPH_CLOSE, k_smooth);
                    cv::morphologyEx(roi_bot, roi_bot, cv::MORPH_CLOSE, k_smooth);
            
                    cv::Mat viz_morph = cv::Mat::zeros(rgb_clean_for_viz.size(), CV_8UC3);
                    viz_morph.setTo(cv::Scalar(0, 0, 255), mask_combined);
                    pub_viz_step3_morph_.publish(cv_bridge::CvImage(depth_msg->header, "bgr8", viz_morph).toImageMsg());
            
                    // --- [Step 4] Cross-Modal Fusion ---
                    cv::Mat viz_fusion = rgb_clean_for_viz.clone(); 
                    cv::Mat overlay = viz_fusion.clone();
                    overlay.setTo(cv::Scalar(0, 255, 0), mask_combined);
                    cv::addWeighted(overlay, 0.4, viz_fusion, 0.6, 0, viz_fusion); 
            
                    {
                        std::lock_guard<std::mutex> lock(lidar_mutex_);
                        if (latest_high_intensity_cloud_ && !latest_high_intensity_cloud_->points.empty()) {
                            for (const auto& p : latest_high_intensity_cloud_->points) {
                                gtsam::Point3 p_w(p.x, p.y, p.z);
                                gtsam::Point3 p_c_raw = world_to_cam_raw.transformTo(p_w);
                                gtsam::Point3 p_c_final = R_total.rotate(p_c_raw);
                                
                                if (p_c_final.z() < 0.2) continue;
            
                                int u = std::round(p_c_final.x() * fx / p_c_final.z() + cx);
                                int v = std::round(p_c_final.y() * fy / p_c_final.z() + cy);
            
                                if (u >= 0 && u < viz_fusion.cols && v >= 0 && v < viz_fusion.rows) {
                                    if (mask_combined.at<uchar>(v, u) > 0) {
                                        cv::circle(viz_fusion, cv::Point(u, v), 3, cv::Scalar(0, 0, 255), -1); 
                                    } else {
                                        cv::circle(viz_fusion, cv::Point(u, v), 2, cv::Scalar(180, 180, 180), -1); 
                                    }
                                }
                            }
                        }
                    }
                    pub_viz_step4_lidar_.publish(cv_bridge::CvImage(depth_msg->header, "bgr8", viz_fusion).toImageMsg());
                }
                // ================= [PAPER VISUALIZATION PIPELINE END] =================
                

                // 1. 判断当前是否静止 (利用里程计读数)

                bool is_stationary_now = (std::abs(semantic_odom_.wheel_speed_) < 0.02 && 
                std::abs(semantic_odom_.wheel_omega_) < 0.02);
                
                // [修复] 直接把 rgb_img 传进去，这样检测器画的绿线就会保存在 rgb_img 上
                VanishingPointDetector::VPResult vp_res = semantic_odom_.vp_detector_.detect(rgb_img, is_stationary_now);
                
                // [新增] 视觉特征提取结束时间 (t2)
                auto t2 = std::chrono::high_resolution_clock::now();
            
                // ========================= [VP 失效检查] =========================
                float center_depth_sum = 0.0;
                int center_count = 0;
                
                int depth_cx = cv_depth_ptr->image.cols / 2;
                int depth_cy = cv_depth_ptr->image.rows / 2;
                int check_radius = 40; 
                
                for(int r = depth_cy - check_radius; r <= depth_cy + check_radius; ++r) {
                    for(int c = depth_cx - check_radius; c <= depth_cx + check_radius; ++c) {
                        if (r >= 0 && r < cv_depth_ptr->image.rows && c >= 0 && c < cv_depth_ptr->image.cols) {
                            unsigned short d_val = cv_depth_ptr->image.at<unsigned short>(r, c);
                            if (d_val > 100 && d_val < 10000) { 
                                center_depth_sum += d_val * 0.001f;
                                center_count++;
                            }
                        }
                    }
                ...
                float avg_center_depth = (center_count > 0) ? (center_depth_sum / center_count) : 0.0;
                
                bool is_too_close_to_wall = (avg_center_depth > 0.1 && avg_center_depth < config.vp_min_activation_dist); 
                
                if (is_too_close_to_wall) {
                    vp_res.valid = false;
                    vp_res.floor_poly.clear(); 
                    cv::putText(rgb_img, "[VP DISABLED] TOO CLOSE", cv::Point(50, 50), 
                                cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2);
                } else {
                    if (semantic_odom_.wheel_speed_ < -0.05) {
                            cv::putText(rgb_img, "[MODE] REVERSING (VP ACTIVE)", cv::Point(50, 80), 
                                        cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);
                    }
                }
            
                cv::Mat floor_mask = cv::Mat::zeros(depth_img.size(), CV_8UC1);
                if (vp_res.valid && !vp_res.floor_poly.empty()) {
                    std::vector<std::vector<cv::Point>> contours = {vp_res.floor_poly};
                    cv::fillPoly(floor_mask, contours, cv::Scalar(255));
                    cv::Mat overlay; rgb_img.copyTo(overlay);
                    cv::fillPoly(overlay, contours, cv::Scalar(0, 255, 0));
                    cv::addWeighted(overlay, 0.25, rgb_img, 0.75, 0, rgb_img);
                    cv::polylines(rgb_img, contours, true, cv::Scalar(0, 255, 0), 2);
                }
            
                int sample_x = (hsv_img.cols - 40) / 2; int sample_y = hsv_img.rows - 30;
                cv::Rect ground_roi(sample_x, sample_y, 40, 20);
                ...
                cv::Mat obstacle_status_map = cv::Mat::zeros(depth_img.size(), CV_8UC1);
            
                float fx = config.cam_fx; float fy = config.cam_fy; 
                float cx = config.cam_cx; float cy = config.cam_cy; 
                int step = 5; float color_similarity_thresh = 40.0; 
                
                octomap::point3d sensor_origin(transform.getOrigin().x(), transform.getOrigin().y(), transform.getOrigin().z());
            
                cleanTemporalBuffer(depth_msg->header.stamp);
            
                // [Mapping High Load Loop 1]
                #pragma omp parallel for collapse(2) schedule(dynamic)
                for (int v = step; v < depth_img.rows - step; v += step) {
                    for (int u = step; u < depth_img.cols - step; u += step) {
                        if (u < 0 || u >= depth_img.cols || v < 0 || v >= depth_img.rows) continue;
                        ...
                        if (floor_mask.at<uchar>(v, u) > 0) is_obstacle = false; 
                        else if (z_academic < config.z_obstacle_thresh) is_obstacle = true;
                        
                        if (is_obstacle && d < 1500) {
                            cv::Vec3b pixel_color = hsv_img.at<cv::Vec3b>(v, u);
                            float diff = colorDist(pixel_color, ground_color);
                            if (p_w.z() < 0.15 && diff < color_similarity_thresh) is_obstacle = false; 
                        }
                        obstacle_status_map.at<uchar>(v, u) = is_obstacle ? 2 : 1;
                    }
                }
            
                for (int v = step; v < depth_img.rows - step; v += step) {
                    for (int u = step; u < depth_img.cols - step; u += step) {
                        uchar status = obstacle_status_map.at<uchar>(v, u);
                        if (status == 2) cv::circle(debug_depth_viz, cv::Point(u, v), 3, cv::Scalar(0, 0, 255), -1);
                        else if (status == 1) cv::circle(debug_depth_viz, cv::Point(u, v), 2, cv::Scalar(0, 255, 0), -1);
                    }
                }
            
                cv::Mat processed_obstacle_img = enhanceImageAdaptive(debug_depth_viz);
                cv::Mat cage_leg_mask;
                cv::inRange(processed_obstacle_img, cv::Scalar(0, 0, 200), cv::Scalar(50, 50, 255), cage_leg_mask);
                
                gtsam::Cal3_S2 camera_K(config.cam_fx, config.cam_fy, 0.0, config.cam_cx, config.cam_cy);
            
                {
                    std::lock_guard<std::mutex> lock(lidar_mutex_);
                    if (!latest_high_intensity_cloud_->points.empty()) {
                        for (const auto& p : latest_high_intensity_cloud_->points) {
                            gtsam::Point3 p_w(p.x, p.y, p.z);
                            gtsam::Point3 p_c_raw = world_to_cam_raw.transformTo(p_w); 
                            gtsam::Point3 p_c_final = R_total.rotate(p_c_raw);
            
                            if (p_c_final.z() < 0.5 || p_c_final.z() > 10.0) continue;
            
                            int u = std::round(p_c_final.x() * camera_K.fx() / p_c_final.z() + camera_K.px());
                            int v = std::round(p_c_final.y() * camera_K.fy() / p_c_final.z() + camera_K.py());
                            
                            semantic_odom_.updateVisualCalibration(u, v, cage_leg_mask, rgb_img);
            
                            if (u >= 0 && u < rgb_img.cols && v >= 0 && v < rgb_img.rows) {
                                cv::circle(rgb_img, cv::Point(u, v), 3, cv::Scalar(255, 0, 0), -1);
                            }
                        }
                        semantic_odom_.period_estimator_.addPoints(
                            latest_high_intensity_cloud_,   
                            T_w_c_raw, 
                            cage_leg_mask,                  
                            world_to_cam_raw,                   
                            camera_K                        
                        );
                        semantic_odom_.period_estimator_.updateEstimate();
                    }
                }
                ...
                bool has_lidar_data = false;
                {
                    std::lock_guard<std::mutex> lock(lidar_mutex_);
                    if (!latest_high_intensity_cloud_->points.empty()) {
                        kdtree.setInputCloud(latest_high_intensity_cloud_);
                        has_lidar_data = true;
                    }
                }
                ...
                std::vector<std::vector<UpdateRequest>> thread_updates(max_threads);
                std::vector<std::vector<cv::Point2f>> thread_obstacle_pts(max_threads);
                
                for(auto& vec : thread_updates) vec.reserve(2000);
                for(auto& vec : thread_obstacle_pts) vec.reserve(2000);
            
                // [Mapping High Load Loop 2]
                {
                    ...
                    for (int v = step; v < processed_obstacle_img.rows - step; v += step) {
                        for (int u = step; u < processed_obstacle_img.cols - step; u += step) {
                            if (u < 0 || u >= processed_obstacle_img.cols || v < 0 || v >= processed_obstacle_img.rows) continue;
            
                            int tid = omp_get_thread_num(); 
            
                            cv::Vec3b pixel = processed_obstacle_img.at<cv::Vec3b>(v, u);
                            
                            if (pixel[2] > 200 && pixel[1] < 100 && pixel[0] < 100) { 
                                if (u < 0 || u >= depth_img.cols || v < 0 || v >= depth_img.rows) continue;
                                ...
                                if (octree_->coordToKeyChecked(endpoint, key)) {
                                    bool is_dynamic = isDynamicObject(sensor_origin, endpoint);
                                    thread_updates[tid].push_back({u, v, endpoint, key, is_dynamic, depth_msg->header.stamp});
                                }
                            }
                        }
                    }
                ...
                // [关键修改] 变量定义在锁的外面，确保函数末尾能访问到
                int total_dynamic_rejected = 0; 
                int total_static_inserted = 0;

                {
                    std::unique_lock<std::shared_mutex> map_lock(map_mutex_); 
            
                    for (int t = 0; t < max_threads; ++t) {
                        for (const auto& req : thread_updates[t]) {
                            
                            bool is_noise = false;           
                            bool confirmed_dynamic = false;  
                            bool confirmed_static = false;   
            
                            if (req.is_geo_dynamic) {
                                TemporalInfo& info = temporal_buffer_[req.key]; 
                                
                                if (info.hit_count == 0) {
                                    info.first_seen_time = req.timestamp;
                                    info.last_view_pos = gtsam::Point3(sensor_origin.x(), sensor_origin.y(), sensor_origin.z());
                                    info.distinct_views = 1;
                                }
                                
                                info.hit_count++;
                                info.last_seen_time = req.timestamp;
            
                                gtsam::Point3 current_sensor_pos(sensor_origin.x(), sensor_origin.y(), sensor_origin.z());
                                double dist_moved = (current_sensor_pos - info.last_view_pos).norm();
            
                                if (dist_moved > 0.2) {
                                    info.distinct_views++;
                                    info.last_view_pos = current_sensor_pos;
                                }
            
                                double duration = (req.timestamp - info.first_seen_time).toSec();
            
                                if (info.hit_count < config.temporal_hit_thresh || info.distinct_views < config.temporal_trust_thresh) {
                                    is_noise = true; 
                                } else {
                                    if (duration < config.temporal_static_life) {
                            ...
                            bool add_to_local_planner = false;
                            bool update_global_map = false;
            
                            if (!is_noise) {
                                if (confirmed_dynamic) {
                                    add_to_local_planner = true; 
                                    total_dynamic_rejected++; // 统计
                                    cv::circle(processed_obstacle_img, cv::Point(req.u, req.v), 2, cv::Scalar(0, 255, 0), -1);
                                }
                                else if (confirmed_static) {
                                    add_to_local_planner = true;
                                    update_global_map = true; 
                                    total_static_inserted++; // 统计
                                }
                            }
            
                            if (add_to_local_planner && has_lidar_data) {
                                pcl::PointXYZI searchPoint; 
                                searchPoint.x = req.endpoint.x(); 
                                searchPoint.y = req.endpoint.y(); 
                                searchPoint.z = req.endpoint.z();
                                std::vector<int> idx; std::vector<float> dist;
                                if (kdtree.radiusSearch(searchPoint, verify_radius_, idx, dist) <= 0) {
                                    add_to_local_planner = false; 
                                }
                            } 
            
                            if (add_to_local_planner) {
                                thread_obstacle_pts[t].push_back(cv::Point2f(req.endpoint.x(), req.endpoint.y())); 
                            }
            
                            if (update_global_map) {
                                float& prob = semantic_probs_[req.key];
                                prob += config.semantic_hit_prob; 
                                if (prob > 1.0) prob = 1.0; 
                                
                                octomap::ColorOcTreeNode* n = octree_->updateNode(req.endpoint, true);
                                if (n) {
                ...
                size_t total_pts = 0;
                for(const auto& vec : thread_obstacle_pts) total_pts += vec.size();
                obstacle_pts.reserve(total_pts);
                for(const auto& vec : thread_obstacle_pts) {
                    obstacle_pts.insert(obstacle_pts.end(), vec.begin(), vec.end());
                }
            
                if (obstacle_pts.size() > 10 || vp_res.valid) {
                    fitNonlinearCenterLine(obstacle_pts, depth_msg->header, processed_obstacle_img, rgb_img, transform, vp_res);
                }
            
                int h = processed_obstacle_img.rows; int w = processed_obstacle_img.cols;
                int roi_h = h * config.roi_h_ratio; int roi_w = w * config.roi_w_ratio; int roi_y = h - roi_h - 5;   
                cv::Rect roi_L(5, roi_y, roi_w, roi_h); cv::Rect roi_R(w - roi_w - 5, roi_y, roi_w, roi_h);
                
                auto calculateOccupancyRatio = [](const cv::Mat& roi) {
                    int red = 0;
                    for(int i=0; i<roi.rows; ++i) {
                        for(int j=0; j<roi.cols; ++j) {
                            if(roi.at<cv::Vec3b>(i,j)[2] > 200 && roi.at<cv::Vec3b>(i,j)[1] < 50) red++; 
                        }
                    }
                    return (float)red / (roi.rows * roi.cols);
                };
                float ratio_L = calculateOccupancyRatio(processed_obstacle_img(roi_L));
                float ratio_R = calculateOccupancyRatio(processed_obstacle_img(roi_R));
                double measurement = std::max(ratio_L, ratio_R); 
            
                semantic_odom_.calibrator_.printDiagnostics();
            
                if (has_vel) {
                    char buff[100];
                    sprintf(buff, "Flow: %.2f %.2f (Conf: %.2f)", v_flow.x(), v_flow.y(), flow_conf);
                    cv::putText(rgb_img, buff, cv::Point(10, 80), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(255,255,0), 2);
                }
            
                // [新增] 优化开始时间 (t3) - 注意：此前包含了耗时的 Mapping 部分
                auto t3 = std::chrono::high_resolution_clock::now();
            
                semantic_odom_.update(depth_msg->header.stamp, measurement, vp_res.valid, vp_res.yaw, vp_res.pt, vp_res.inliers, 
                    has_vel, v_flow, flow_conf, flow_depth);
                
                // [新增] 优化结束时间 (t4)
                auto t4 = std::chrono::high_resolution_clock::now();
                
                semantic_odom_.drawVisualization(rgb_img, measurement, vp_res);
                ...
                // [新增] 计算耗时并写入 Table 6 的数据
                if (g_logger.f_time.is_open()) {
                    double ms_pre = std::chrono::duration<double, std::milli>(t1 - t0).count();
                    double ms_vis = std::chrono::duration<double, std::milli>(t2 - t1).count();
                    // t_map: 从 t2 (VP检测后) 到 t3 (优化前)，包含了 Ray-casting 和 Map Update 循环
                    double ms_map = std::chrono::duration<double, std::milli>(t3 - t2).count();
                    double ms_opt = std::chrono::duration<double, std::milli>(t4 - t3).count();
                    double ms_viz = std::chrono::duration<double, std::milli>(t5 - t4).count(); 
                    double ms_total = std::chrono::duration<double, std::milli>(t5 - t0).count();
            
                    g_logger.f_time << std::fixed << std::setprecision(3)
                        << depth_msg->header.stamp.toSec() << " "
                        << ms_pre << " " << ms_vis << " " 
                        << 0.0 << " " // LiDAR Time (在另一个回调中，此处填0占位)
                        << ms_opt << " " << ms_map << " " << ms_total
                        << std::endl;
                }
                // ================= [🔴 请在此处插入 修改点 2 代码] =================

                // [新增] 1. 记录地图生长曲线 (Map Efficiency)
                // 降低频率：每 10 帧 (约 0.5秒) 记录一次即可，不需要太密集
                static int map_log_counter = 0;
                if (map_log_counter++ % 10 == 0 && g_logger.f_map_stats.is_open()) {
                    size_t num_nodes = octree_->size();
                    double mem_usage = octree_->memoryUsage() / 1024.0 / 1024.0; 
                    
                    g_logger.f_map_stats << std::fixed << std::setprecision(3)
                        << depth_msg->header.stamp.toSec() << " "
                        << num_nodes << " "
                        << mem_usage << " "           // ✅ 确保这里有空格
                        << total_dynamic_rejected << " " 
                        << total_static_inserted      // ✅ 补上这个变量
                        << std::endl;
                }
                
                // [新增] 2. 自动保存最终地图 (Auto-Save Snapshot) - 支持 .ot 和 .pcd
                static double last_save_time = 0;
                double curr_time_sec = depth_msg->header.stamp.toSec();
                
                if (last_save_time == 0 || (curr_time_sec - last_save_time > 60.0)) {
                    // ---------------------------------------------------------
                    // 1. 保存 .ot 文件 (OctoMap 格式)
                    // ---------------------------------------------------------
                    std::string map_path_ot = g_logger.base_path + "auto_save_map.ot";
                    try {
                        if (octree_->write(map_path_ot)) {
                            ROS_INFO_THROTTLE(60, "[AutoSave] Map (.ot) saved to %s", map_path_ot.c_str());
                        } else {
                            ROS_WARN_THROTTLE(60, "[AutoSave] Failed to write .ot file");
                        }
                    } catch (...) {
                        ROS_WARN("[AutoSave] Exception during .ot saving.");
                    }

                    // ---------------------------------------------------------
                    // 2. 保存 .pcd 文件 (Point Cloud 格式) - [新增功能]
                    // ---------------------------------------------------------
                    std::string map_path_pcd = g_logger.base_path + "auto_save_map.pcd";
                    ...
                    for(octomap::ColorOcTree::leaf_iterator it = octree_->begin_leafs(); 
                        ...
                        if (octree_->isNodeOccupied(*it)) {
                            ...
                            // 2. 获取颜色 (从 ColorOcTreeNode)
                            // 注意：it->getColor() 返回的是 octomap::ColorOcTreeNode::Color
                            octomap::ColorOcTreeNode::Color c = it->getColor();
                            p.r = c.r;
                            p.g = c.g;
                            p.b = c.b;
                            
                            cloud_to_save.push_back(p);
                        }
                    }
                    ...
                    if (!cloud_to_save.empty()) {
                        try {
                            pcl::io::savePCDFileBinary(map_path_pcd, cloud_to_save);
                            ROS_INFO_THROTTLE(60, "[AutoSave] Map (.pcd) saved to %s (Points: %lu)", 
                                              map_path_pcd.c_str(), cloud_to_save.size());
                        } catch (...) {
                            ROS_WARN("[AutoSave] Failed to save PCD file.");
                        }
                    } else {
                        ROS_WARN_THROTTLE(60, "[AutoSave] Cloud is empty, skipping PCD save.");
                    }
                    
                    last_save_time = curr_time_sec;
                }
            }
            ...
            void lidarCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
                ...
                for (auto& p : pcl_cloud.points) {
                    ...
                    float factor = std::pow(range / config.intensity_ref_dist, config.intensity_alpha);
                    float norm_intensity = p.intensity * factor;
                    p.intensity = norm_intensity; 
                    normalized_cloud->points.push_back(p);
                }
        
                {
                    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_local(new pcl::PointCloud<pcl::PointXYZI>());
                    for(const auto& p : normalized_cloud->points) {
                        if(p.z > -2.0 && p.z < 0.5) cloud_local->points.push_back(p);
                    }
                    auto plane_res = feature_extractor_.extractGroundPlane(cloud_local);
                    if (plane_res.first) {
                        semantic_odom_.updateGroundPlane(plane_res.second);
                    }
                }
                // ================= [🔴 请在此处插入您的代码] =================
                {
                    pcl::PointCloud<pcl::PointXYZRGB> ground_viz_cloud;
                    // 复用您之前提取的 cloud_local (z > -2.0 && z < 0.5)
                    // 或者是直接遍历 normalized_cloud
                    for(const auto& p : normalized_cloud->points) {
                        // 简单的地面判断标准：高度低于 0.15m
                        if(p.z < 0.10 && p.z > -0.7) {
                            pcl::PointXYZRGB p_color;
                            p_color.x = p.x; p_color.y = p.y; p_color.z = p.z;
                            
                            // 设置为一种雅致的深绿色 (Forest Green) 或 灰色
                            // 这种颜色在论文截图中作为背景非常合适
                            p_color.r = 50; 
                            p_color.g = 160; 
                            p_color.b = 50; 
                            
                            ground_viz_cloud.points.push_back(p_color);
                        }
                    }

                    // [新增] 打印日志：看看到底提取了多少个地面点？
                    if (ground_viz_cloud.empty()) {
                        ROS_WARN_THROTTLE(1.0, "[Ground Viz] 0 points found! Z range (%.2f, %.2f) is wrong?", -3.0, 0.3);
                    } else {
                        ROS_INFO_THROTTLE(1.0, "[Ground Viz] Published %lu points. Color: RGB(50,160,50)", ground_viz_cloud.size());
                    }

                    if (!ground_viz_cloud.empty()) {
                        sensor_msgs::PointCloud2 output_msg;
                        pcl::toROSMsg(ground_viz_cloud, output_msg);
                        output_msg.header = msg->header; // 保持时间戳和坐标系一致
                        // 确保在全局坐标系显示，或者保持 sensor frame
                        // 这里建议直接用 msg->header.frame_id，因为 normalized_cloud 是在 sensor frame 下的
                ...
                pcl::PointCloud<pcl::PointXYZI>::Ptr adaptive_high_intensity = filterHighIntensityAdaptive(normalized_cloud);
                pcl::PointCloud<pcl::PointXYZI>::Ptr clustered_cloud = filterNoiseByClustering(adaptive_high_intensity);
                pcl::PointCloud<pcl::PointXYZI>::Ptr final_feature_cloud = feature_extractor_.filterByLinearity(clustered_cloud);
        
                ROS_INFO_THROTTLE(2.0, "[LiDAR Features] Raw: %lu -> Adaptive: %lu -> Clustered: %lu -> Final: %lu", 
                    normalized_cloud->size(), adaptive_high_intensity->size(), clustered_cloud->size(), final_feature_cloud->size());
        
                if (!final_feature_cloud->points.empty()) {
                    std::lock_guard<std::mutex> lock(lidar_mutex_);
                    tf::StampedTransform tf_sensor;
                    try {
                        // [修改] 非阻塞 TF 检查
                        if (!tf_listener_.canTransform(config.global_frame_id, msg->header.frame_id, msg->header.stamp)) {
                            return; 
                        }
                    ...
                    pcl::PointCloud<pcl::PointXYZI> global_geo_cloud;
                    for(const auto& p : final_feature_cloud->points) {
                        tf::Vector3 p_local(p.x, p.y, p.z);
                        tf::Vector3 p_global = tf_sensor * p_local;
                        pcl::PointXYZI p_g; 
                        p_g.x = p_global.x(); p_g.y = p_global.y(); p_g.z = p_global.z(); p_g.intensity = p.intensity;
                        global_geo_cloud.points.push_back(p_g);
                    }
                    *latest_high_intensity_cloud_ = global_geo_cloud;
                }
        
                {
                    pcl::PointCloud<pcl::PointXYZI>::Ptr wall_candidates(new pcl::PointCloud<pcl::PointXYZI>());
                    for(const auto& p : normalized_cloud->points) {
                        if(p.z > 0.5 && p.z < 2.0 && p.intensity > 15.0) {
                            if (std::abs(p.y) > 0.5 && std::abs(p.y) < 3.0) {
                                wall_candidates->points.push_back(p);
                            }
                        }
                    }
                    
                    bool wall_detected = feature_extractor_.extractVerticalPlane(wall_candidates, 0.15);
                    semantic_odom_.setWallDetected(wall_detected);
        
                    auto line_res = feature_extractor_.extractCorridorLine(wall_candidates);
                    if (line_res.valid) {
                        semantic_odom_.updateLineFeature(line_res);
                    }

                    // 2. [新增] 正面墙检测 (专门用于屏蔽光流)
                    pcl::PointCloud<pcl::PointXYZI>::Ptr front_candidates(new pcl::PointCloud<pcl::PointXYZI>());
                    for(const auto& p : normalized_cloud->points) {
                        // 筛选前方区域：X [1.5, 6.0], Y [-1.0, 1.0] (走廊中央)
                        if(p.z > 0.5 && p.z < 2.0 && p.intensity > 15.0) {
                            if (p.x > 1.5 && p.x < 6.0 && std::abs(p.y) < 1.0) {
                                front_candidates->points.push_back(p);
                            }
                        }
                    }
                    // 使用同样的平面提取算法
                    bool front_detected = feature_extractor_.extractVerticalPlane(front_candidates, 0.15);
                    semantic_odom_.setFrontWallDetected(front_detected);
                }
